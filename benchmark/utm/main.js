...
function main() {
    ...
    let logger = Util.getLogger('benchmark/utm/main.js');
    let absConfigFile;
    if (typeof configFile === 'undefined') {
        absConfigFile = path.join(__dirname, 'config-linear-rate.yaml');
    } else {
        absConfigFile = path.isAbsolute(configFile)
            ? configFile
    ...
    if (typeof networkFile === 'undefined') {
        try {
            absNetworkFile = Util.resolvePath('network/utm/utm-config.json');
        } catch (err) {
            logger.error(
                'failed to find blockchain.config in ' + absConfigFile
            );
            return;
        }
    } else {
        absNetworkFile = path.isAbsolute(networkFile)
            ? networkFile
            : path.join(__dirname, networkFile);
    }
    ...
    (async () => {
        try {
            ...
            logger.error(
                `Error while executing the benchmark: ${
